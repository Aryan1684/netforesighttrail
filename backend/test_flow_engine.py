...
def packet(src,dst,sport,dport,t,flags="0x0010",length=100):
    tcp=SimpleNamespace(srcport=str(sport),dstport=str(dport),flags=flags,window_size_value="65535",seq_raw="1",ack_raw="1")
    ip=SimpleNamespace(src=src,dst=dst,ttl="64")
    return SimpleNamespace(sniff_timestamp=str(t),length=str(length),ip=ip,tcp=tcp)
